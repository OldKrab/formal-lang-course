...
def test_query_to_one_path():
    ...
    query = "abc"
    pairs = fa_utils.query_regex_to_fa_with_states(fa, {0}, {3}, query)
    assert len(pairs) == 1
    assert (0, 3) in pairs
...
def test_query_with_multiple_starts():
    ...
    query = "(ad|cd)"
    pairs = fa_utils.query_regex_to_fa_with_states(fa, {0, 1, 2}, {4}, query)
    assert len(pairs) == 2
    assert (0, 4) in pairs
    assert (2, 4) in pairs
...
def test_query_with_multiple_finals():
    ...
    query = "(ab|ad)"
    pairs = fa_utils.query_regex_to_fa_with_states(fa, {0}, {2, 3, 4}, query)
    assert len(pairs) == 2
    assert (0, 2) in pairs
    assert (0, 4) in pairs
...
def test_query_with_loops():
    ...
    query = "(abbb|cddd)"
    pairs = fa_utils.query_regex_to_fa_with_states(fa, {0}, {1, 2, 3}, query)
    assert len(pairs) == 2
    assert (0, 1) in pairs
    assert (0, 2) in pairs
